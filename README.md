...
**Stack:** Node.js / Express 5 · React · SQLite (better-sqlite3) · Python subprocess ML (WD Tagger, BLIP) · JWT auth · Socket.io
**Status:** Active
...
Artifex is a full-stack application split into a Node.js backend and a React frontend. When images are uploaded, a background job queue processes them through multiple ML pipelines: WD Tagger and a ViT-based classifier tag content, and BLIP generates natural language captions. All metadata is indexed in SQLite for fast full-text search.
...
- **ML Classification**: ViT-based content classifier via @huggingface/transformers in Node
...
- **Auto-tagging**: Images are automatically classified with content tags via WD Tagger (SwinV2) and a ViT-based classifier
...
backend/
  ...
  lib/
    ...
    nsfwClassifier.js # Node-based ViT content classifier
